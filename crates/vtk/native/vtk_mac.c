...
_Bool vtk_window_init_platform(struct VtkWindowNative *vtk_window) {
  ...
  id window =
      [[[NSWindow alloc] initWithContentRect:frame styleMask:windowStyle backing:NSBackingStoreBuffered defer:NO]
          autorelease];
  [window autorelease];
  ...
  CALL_VK(vkCreateMetalSurfaceEXT(vtk_window->vtk_device->vk_instance, &surface_create_info, NULL,
                                  &vtk_window->vk_surface));
